...
async function exportTabunganFilter() {
    ...
    // Kolom kanan
    doc.line(8.3, yStart, 8.3, yStart + 6.6);
    doc.line(10.2, yStart, 10.2, yStart + 6.6);
    doc.line(12.8, yStart, 12.8, yStart + 6.6);
    ...
    doc.text("TGL", 7.7, yStart + 0.4);
    doc.text("SETOR", 9.0, yStart + 0.4);
    doc.text("SALDO", 11.5, yStart + 0.4);
    ...
    if (data.saldoAwal) {
        let saldoAwalBersih = String(data.saldoAwal)
            .replace(/[^0-9-]/g, "");

        berjalan = parseFloat(saldoAwalBersih) || 0;
    }

    const listTransaksiSemua = data.data.sort(
    ...
    listTransaksiSemua.forEach((r) => {
        ...
        const tglAngka = String(r.tanggal).includes("/")
            ? parseInt(r.tanggal.split("/")[0])
            : tglObj.getDate();

        let nominalBersih = String(r.nominal || "0")
            .replace(/[^0-9-]/g, "");
        ...
        if (tanggalValue) {
            ...
            const tglFilterAngka =
                parseInt(tanggalValue.split("-")[2]) ||
                parseInt(tanggalValue);
        ...
        const nominalTxt =
            "Rp " + nilaiNominal.toLocaleString("id-ID");

        const saldoTxt =
            "Rp " + berjalan.toLocaleString("id-ID");
        ...
        if (barisCetak <= 16) {
            ...
            doc.text(nominalTxt, 5.5, yL, {
                align: "right"
            });

            doc.text(saldoTxt, 7.2, yL, {
                align: "right"
            });
        ...
        // =========================
            ...
            doc.text(nominalTxt, 12.5, yR, {
                align: "right"
            });

            doc.text(saldoTxt, 14.2, yR, {
                align: "right"
            });
